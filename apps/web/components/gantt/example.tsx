...
const Example = () => {
	...
	return (
		<div className="h-full w-full">
			<GanttProvider
				className="h-full rounded-lg border bg-muted p-2 shadow-lg"
				onAddItem={handleAddFeature}
				range="monthly"
				zoom={100}
			>
				<GanttSidebar className="mr-2 rounded-lg bg-background/80 p-4 shadow">
					{Object.entries(sortedGroupedFeatures).map(([group, features]) => (
						<GanttSidebarGroup key={group} name={group} className="mb-4">
							{features.map((feature) => (
								<GanttSidebarItem
									feature={feature}
									key={feature.id}
									onSelectItem={handleViewFeature}
									className="mb-2 rounded bg-muted p-2 transition hover:bg-accent"
								/>
							))}
						</GanttSidebarGroup>
					))}
				</GanttSidebar>
				<GanttTimeline className="rounded-lg bg-background p-4 shadow">
					<GanttHeader />
					<GanttFeatureList>
						{Object.entries(sortedGroupedFeatures).map(([group, features]) => (
							<GanttFeatureListGroup key={group} className="mb-4">
								{features.map((feature) => (
									<div className="mb-2 flex" key={feature.id}>
										<ContextMenu>
											<ContextMenuTrigger asChild>
												...
												>
													<GanttFeatureItem
														...
														{...feature}
														className="rounded bg-muted p-2 transition hover:bg-accent"
													>
