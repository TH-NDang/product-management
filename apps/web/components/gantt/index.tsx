...
const getDateByMousePosition = (context: GanttContextProps, mouseX: number) => {
	const yearGantt = context.timelineData[0]?.year ?? new Date().getFullYear();
	const timelineStartDateGantt = new Date(yearGantt, 0, 1);
	const columnWidth = (context.columnWidth * context.zoom) / 100;
	const offset = Math.floor(mouseX / columnWidth);
	const daysIn = getsDaysIn(context.range);
	const addRange = getAddRange(context.range);
	const month = addRange(timelineStartDateGantt, offset);
...
export const GanttFeatureItem: FC<GanttFeatureItemProps> = ({
	...
	const [scrollX] = useGanttScrollX();
	const gantt = useContext(GanttContext);
	const yearFeature = gantt.timelineData[0]?.year ?? new Date().getFullYear();
	const timelineStartDateFeature = new Date(yearFeature, 0, 1);
	...
	const offset = useMemo(
		() => getOffset(startAt, timelineStartDateFeature, gantt),
		[startAt, timelineStartDateFeature, gantt],
	);
...
> = memo(({ label, date, id, onRemove, className }) => {
	...
	const yearMarker = gantt.timelineData[0]?.year ?? new Date().getFullYear();
	const timelineStartDateMarker = new Date(yearMarker, 0, 1);

	// Memoize expensive calculations
	const offset = useMemo(
		() => differenceIn(date, timelineStartDateMarker),
		[differenceIn, date, timelineStartDateMarker],
	);
...
export const GanttProvider: FC<GanttProviderProps> = ({
	...
	const scrollToFeature = useCallback(
		(feature: GanttFeature) => {
			...
			const yearScroll = timelineData[0]?.year ?? new Date().getFullYear();
			const timelineStartDateScroll = new Date(yearScroll, 0, 1);

			// Calculate the horizontal offset for the feature's start date
			const offset = getOffset(feature.startAt, timelineStartDateScroll, {
				zoom,
				range,
				columnWidth,
				sidebarWidth,
